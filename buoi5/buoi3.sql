...
from congviec c
left join dean d on c.mada=d.mada
where tenda=''
;
...
where p.mada in (
				 select d.mada
                 from dean d
                 left join diadiem_phg dd on d.phong=dd.maphg
                 where ddiem_da='TP HCM'
                 and diadiem != 'TP HCM');


select  concat(honv,' ',tenlot,' ',tenv),dchi,n.phg
from nhanvien n
where n.phg in (select d.phong
				from dean d
                left join diadiem_phg dd on d.phong=dd.maphg
                where ddiem_da='TP HCM'
                and diadiem != 'TP HCM');
...
-- 54. cho biết nhân viên nào làm việc vất vả nhất
select concat(honv,' ',tenlot,' ',tenv)
from nhanvien
where manv in (
				select sum(thoigian),ma_nvien
                from phancong 
                group by ma_nvien
                order by sum(thoigian) desc
				);
SELECT n.honv, n.tenlot, n.tenv, n.manv, SUM(p.thoigian) AS total_time
FROM nhanvien n
JOIN phancong p ON n.manv = p.ma_nvien
GROUP BY n.honv, n.tenlot, n.tenv, n.manv
